...
=== 任务调度策略
  ...
  RocketOS的任务调度系统采用了多优先级队列调度和完全公平调度(CFS, Completely Fair Scheduler)两种主要的调度策略来满足不同场景下的性能需求。多优先级队列调度是一种传统而高效的调度算法，通过为任务分配不同的优先级，确保高优先级任务能够优先获得CPU资源。这种策略特别适用于实时系统和对响应时间有严格要求的应用场景，能够保证关键任务的及时执行。

  完全公平调度则代表了现代操作系统调度的另一种思路，它通过虚拟运行时间的概念来确保所有任务获得公平的CPU时间分配，根据任务的虚拟运行时间始终选择虚拟运行时间最小的任务执行。这种策略能够有效避免任务饥饿现象，提供良好的交互性能，特别适合桌面环境和服务器工作负载。
  ...
  #code-figure(
    ...
    ```,
    caption: [多优先级队列调度设计],
    label-name: "Multi-level Priority Queue",
  )

  #h(2em)FIFOScheduler是RocketOS中多优先级队列调度策略的核心实现，采用分层队列架构来满足不同类型任务的调度需求。调度器维护两组独立的优先级队列：包含100个优先级的实时任务队列(rt_queues)和包含40个优先级的普通任务队列(normal_queues)，确保实时任务始终能够抢占普通任务执行。每个队列使用VecDeque数据结构，提供O(1)时间复杂度的队首队尾操作。
  
  调度器的核心优化在于其位图索引机制，通过rt_bitmap(128位)和normal_bitmap(64位)来跟踪各优先级队列的状态。当队列非空时对应位被置1，使得调度器能够通过位运算在常数时间内定位到最高优先级的非空队列，避免了遍历所有队列的性能开销。配合task_index这一BTreeMap索引结构，调度器实现了从任务ID到队列位置的快速映射，支持高效的任务查找、移除和优先级调整操作。
  ...
  #code-figure(
    ...
    ```,
    caption: [CFS队列调度设计],
    label-name: "CFS Queue",
  )

  #h(2em)CFSScheduler实现了完全公平调度算法，通过虚拟运行时间机制确保系统中所有任务获得公平的CPU资源分配。调度器的核心数据结构tasks_timeline采用BTreeMap来模拟红黑树的应用，调度器通过load字段维护当前运行队列的负载权重信息，用于计算各任务分配的运行时间。LoadWeight结构记录了队列中所有任务的权重总和，当任务加入或离开调度队列时，调度器会相应更新这个值。负载权重的准确计算直接影响到虚拟运行时间的推进速度和任务时间片的分配，是实现公平调度的基础。nr_running字段则实时跟踪当前可运行任务的数量，为调度决策提供重要的统计信息，同时也是计算调度周期和时间片长度的重要依据。

  虚拟运行时间(vruntime)是CFS调度器实现公平性的核心机制，其计算公式为：
      $ text("vruntime") = text("delta_exec") * (text("NICE_0_LOAD") / text("weight")) $
  #h(2em)其中delta_exec是任务实际运行的时间，NICE_0_LOAD是标准权重值(通常为1024)，weight是任务的权重。这个公式确保了高权重任务的vruntime推进较慢，从而获得更多的CPU时间；而低权重任务的vruntime推进较快，相对获得较少的执行机会。当任务被唤醒或新创建时，调度器会将其vruntime设置为当前运行队列的最小vruntime值，避免新任务因为较小的vruntime值而垄断CPU资源。通过虚拟运行时间机制，调度器能够准确跟踪每个任务的相对执行进度，根据任务权重动态调整调度决策，从而避免了传统优先级调度中可能出现的任务饥饿问题。
...
=== 多核支持
  ...
  在RocketOS的架构设计中，我们采用了核心绑定的设计策略来实现多核处理器的实现与管理。它能够有效提升缓存利用效率，当任务固定在特定核心上执行时，避免了任务在不同核心间迁移时频繁的缓存失效和重新加载，从而减少内存访问延迟并提高整体执行效率

  绑核策略还简化了系统的调度复杂度并提供了更精细的资源管理能力。相比于复杂的负载均衡算法，绑核方式减少了调度器的决策开销，这种设计使得系统能够为不同类型的工作负载提供差异化的服务质量，实现更加精准的性能调优和资源优化配置。
...
=== 任务控制块设计
  ...
  #code-figure(
    ...
    pub struct Task {
      ...
      restore_mask: AtomicBool,                       // 是否恢复掩码
      itimerval: Arc<RwLock<[ITimerVal; 3]>>,         // 定时器
      timers: 
      Arc<Mutex<[PosixTimer; MAX_POSIX_TIMER_COUNT]>>,// POSIX定时器
      ...
      #[cfg(feature = "cfs")]
      sched_entity: Arc<CFSSchedEntity>,              // CFS调度实体
  ...
  #code-figure(
    ...
    ```,
    caption: [任务控制块内部结构],
    label-name: "任务控制块内部结构",
  )

  #h(2em)在关于进程与线程的架构设计上，RocketOS采取了与Linux相类似的先进设计理念。Linux内核通过sys_clone系统调用的灵活标志位机制，实现了对不同类型任务创建的统一管理框架。通过传递不同的flags参数组合（如CLONE_VM、CLONE_FILES、CLONE_SIGHAND等），同一个系统调用既可以创建拥有完全独立虚拟地址空间和系统资源的传统进程，也可以生成与父任务共享特定资源（如内存空间、文件描述符或信号处理器）的轻量级线程。这种统一而灵活的设计充分体现了操作系统设计中"机制与策略分离"的核心思想，为上层应用提供了丰富的进程/线程创建选择。

  基于高效的调度框架，RocketOS采用了统一的Task结构体设计模式，通过Rust语言Arc（原子引用计数）与Mutex（互斥锁）的精妙结合，优雅地解决了多线程环境下的资源共享与并发访问挑战。这种设计架构不仅从根本上保证了内存安全性，消除了传统操作系统中常见的竞态条件和数据竞争问题，还通过零拷贝的资源共享机制实现了显著的性能提升。Arc的引用计数特性使得任务对象能够在多个调度器、系统调用处理器和内核子系统之间安全共享，而无需进行昂贵的深度拷贝操作。

  针对Task结构体中访问频率较低的字段，RocketOS采用了粗粒度锁定策略，通过一把大锁来统一管理这些不常用的元数据。这种设计决策基于对系统运行时行为的深入分析：大部分调度操作只需要访问任务的核心调度信息，如地址空间、任务状态和文件描述符表等热点字段，而诸如任务调度策略、信号栈等冷门字段的访问与修改频率极低。通过将这些字段放在统一的锁保护下，系统避免了为每个字段单独维护锁的内存开销和同步复杂性，在任务创建时显著减少了锁初始化的性能消耗，同时简化了内存布局并提高了缓存局部性。
...
=== 任务状态设计
  ...
  RocketOS的任务状态设计采用了经典的五状态模型，通过TaskStatus枚举类型精确定义了任务在其生命周期中可能处于的各种状态，系统中的每个任务都必然处于这六种状态中的一种:
  #pad(left: 3em)[
    ...
    - *停止（Stopped）*：任务因接收到信号而被暂停，等待继续执行的条件。
  ]

  #figure(
    image("img/任务状态切换图.drawio.png", width: 45%),
    caption: [任务状态切换图]
  ...
  当运行中的任务接收到特定的停止信号（如SIGSTOP或SIGTSTP）时，无论该任务当前正在执行什么操作，都会立即被强制暂停并转入Stopped状态。Stopped状态的恢复机制同样依赖于信号传递。当系统或其他进程向处于Stopped状态的任务发送SIGCONT信号时，该任务会被唤醒并重新获得继续执行的机会。收到SIGCONT信号后，任务会从Stopped状态平滑过渡到Ready状态，重新进入操作系统的调度队列，等待调度器根据调度算法为其分配CPU时间片。
...
=== 用户态 → 内核态切换
  ...
  #algorithm-figure(
    ...
    7:    stvec/CSR_EENTRY ← &__trap_from_kernel
...
=== 内核态 → 内核态切换
  ...
  内核态的切换主要发生在内核下的异常处理过程中。特别是针对loongarch这种对齐敏感的架构，在运行某一内核指令时，若其访问的内存的地址没有处于对齐的位置，则会触发AddressNotAligned异常。此时，系统会跳转到`__trap_from_kernel`标签定义的汇编代码段，保存当前上下文并执行特定的异常处理逻辑。

    #algorithm-figure(
    ```rs
    Input: kernel_context
    Output: processed_context

    1:    sp ← sp - 34 × 8
    2:    for i ∈ SaveSet do  
              mem[sp + i × 8] ← x[i]  #SaveSet = {x0~x31(除sp)}
    3:    mem[sp + 32 × 8] ← sstatus/CSR_PRMD  
          mem[sp + 33 × 8] ← sepc/CSR_ERA  
    4:    call/bl kernel_trap_handler(a0)
    5:    t0 ← mem[sp + 32 × 8]     
          t1 ← mem[sp + 33 × 8]           
        sstatus/CSR_PRMD   ← t0  
        sepc/CSR_ERA       ← t1  
    ...
    7:  sret/ertn
    ```,
    caption: [trap_from_kernel算法],
    label-name: "trap-from-kernel",
  )
