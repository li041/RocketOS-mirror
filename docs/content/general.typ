= 概述
== RocketOS内核整体设计

RocketOS 是一款采用 Rust 语言开发的现代化宏内核操作系统，原生支持 RISC-V 和 LoongArch 两种主流处理器架构。该系统采用同步栈式设计架构，集成了完整的中断处理机制、进程管理系统、内存管理模块、文件系统以及网络协议栈等核心组件，通过系统调用接口为用户程序提供高效可靠的服务支持，详细结构如#[@fig:系统架构] 所示。RocketOS 充分利用 Rust 语言的内存安全特性和零成本抽象能力，在保证系统稳定性的同时实现了出色的内核性能表现，为现代计算环境提供了一个安全、高效、可扩展的操作系统解决方案。

#figure(
  image("img/RocketOS内核架构图.drawio.png", width: 85%),
  caption: [RocketOS内核架构图],
)<系统架构>

== RocketOS当前工作

截至2025年6月30日，RocketOS 已经实现了以下核心功能模块：
#pad(left: 3em)[
- 内存管理模块，提供高效的内存分配和虚拟内存
- 文件系统，支持多种文件操作和目录管理
- 完善的信号机制，支持进程间的信号传递和处理
- 进程管理系统，支持多任务调度和进程间通信
- 网络协议栈，支持 TCP/IP 协议和基本网络通信
]

#h(2em)截至2025-6-30日20点，RocketOS通过除ltp外全部测试点，在实时排行榜中处于榜首，总分共计11720.05

#figure(
  image("img/队伍成绩1.png", width: 70%),
  caption: [RocketOS实时排行榜成绩],
)<score-1>

#figure(
  image("img/队伍成绩2.png", width: 70%),
  caption: [RocketOS实时排行榜成绩],
)<score-2>

== 成员分工

**李昕昊（队长）：** 负责管理系统内存分配与回收，维护文件的存储与访问，实现对不同硬件架构（如RISC-V、LoongArch）的支持，确保系统资源高效稳定运行。

**翁浩男：** 负责创建、调度和销毁进程，管理进程间通信与同步；实现信号机制，支持异步事件通知和处理，保障系统的并发执行和响应能力。

**沈翃宇：** 实现网络协议栈，管理数据包的发送与接收，支持多种网络通信协议，保障内核与外部网络的高效互联和数据传输。

#pagebreak()
