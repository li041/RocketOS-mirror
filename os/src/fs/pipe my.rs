...
impl FileOp for Pipe {
    ...
    fn read<'a>(&'a self, buf: &'a mut [u8]) -> usize {
        ...
        loop {
            ...
            if ret != 0 {
                log::error!("[Pipe::read] read {} bytes", ret);
                return ret;
            } else if self.buffer.lock().eof() {
                // empty buffer and no writer, EOF
                log::error!("[Pipe::read] EOF");
                return 0;
            } else {
                // empty buffer but writer exists, wait
                log::error!("[Pipe::read] empty buffer, wait for writer");
                yield_current_task();
                continue;
            }
            // yield_task().await;
...
impl Drop for Pipe {
    // 不对Pipe在fork后有两个写端, 两个读端
    fn drop(&mut self) {
        if self.writeable {
            log::error!("[Pipe::drop] writer dropped");
            let mut buffer = self.buffer.lock();
            buffer.eof = true;
        }
    }
