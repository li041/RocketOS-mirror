...
impl Ext4InodeDisk {
    ...
    pub fn new_chr(mode: u16, major: u32, minor: u32) -> Box<Self> {
        let mut inode = Ext4InodeDisk::new_bare();
        let current_time = TimeSpec::new_wall_time();
        println!("[new_chr] current_time: {:?}", current_time);
        inode.mode = mode | S_IFCHR;
        println!("[new_chr] after set mode");
        inode.uid = 0;
        ...
        println!("before set time");
        // 设置时间戳
        inode.set_atime(current_time);
        inode.set_ctime(current_time);
        inode.set_mtime(current_time);
        println!("[new_chr] ready to return");
        inode
    }
    // pub fn new_chr(mode: u16, major: u32, minor: u32) -> Box<Ext4InodeDisk> {
    //     let current_time = TimeSpec::new_wall_time();
    //     println!(
    //         "[new_chr] mode: {}, major: {}, minor: {}, current_time: {:?}",
    //         mode, major, minor, current_time
    //     );
    //     let mut block: [u8; 60] = [0; 60];
    //     println!("before set minor and major");
    //     // 设置主设备号和次设备号（低8字节）
    //     block[0..4].copy_from_slice(&major.to_le_bytes());
    //     block[4..8].copy_from_slice(&minor.to_le_bytes());
    //     println!("before set osd2");
    //     let osd2: [u8; 12] = [0; 12]; // 操作系统相关字段

    //     let mut inode = Box::new(Ext4InodeDisk {
    //         mode: mode | S_IFCHR,
    //         uid: 0,
    //         gid: 0,
    //         size_lo: 0,
    ...
    //         // 时间相关字段
    //         atime: current_time.sec as u32,
    //         change_inode_time: current_time.sec as u32,
    //         modify_file_time: current_time.sec as u32,
    //         dtime: 0,
    //     });
    //     println!("[new_chr] returning Box<Ext4InodeDisk>");
    //     inode
    // }
    pub fn new_blk(mode: u16, major: u32, minor: u32) -> Box<Self> {
        let mut inode = Ext4InodeDisk::new_bare();
        let current_time = TimeSpec::new_wall_time();
        println!("[new_blk] current_time: {:?}", current_time);
        inode.mode = mode | S_IFBLK;
        println!("[new_blk] after set mode");
        inode.uid = 0;
    ...
    //     inode.links_count = 1;
    //     inode.flags = 0;
    //     println!("[new_blk] after set uid, gid, size, blocks, links, flags");
    //     // 设置主设备号和次设备号
    //     inode.block[0..4].copy_from_slice(&major.to_le_bytes());
    //     inode.block[4..8].copy_from_slice(&minor.to_le_bytes());
    //     println!("[new_blk] after set major and minor");
    //     // 设置时间戳
    //     inode.set_atime(current_time);
    //     inode.set_ctime(current_time);
    //     inode.set_mtime(current_time);
    //     println!("[new_blk] ready to return");
    //     inode
    // }
}
    ...
    fn lookup_extent(
        ...
        if extent_header.depth > 0 {
            ...
            if let Some(idx) = extent_idxs.iter().find(|idx| idx.block <= current_block) {
                ...
                // 未找到对应的索引节点
                return Box::new(Ext4Extent::default()); // 返回默认值
            }
        }
        // 根节点就是叶子节点
...
impl Ext4Inode {
    ...
    pub fn lookup_extent(&self, page_index: usize) -> Box<Ext4Extent> {
        // 仅用于文件的读
    ...
    pub fn lookup_or_create_extent(
        ...
        let mut inner = self.inner.write();
        let extent = inner.inode_on_disk.lookup_extent(
            logical_start_block,
            block_device.clone(),
            ext4_block_size,
        );
    ...
    pub fn fallocate(&self, mode: FallocFlags, offset: usize, len: usize) -> SyscallRet {
        ...
        if mode.contains(FallocFlags::PUNCH_HOLE) {
            ...
            for block_num in start_block..end_block {
                let extent = inode_on_disk.lookup_extent(
                    block_num as u32,
                    self.block_device.clone(),
                    self.ext4_fs.upgrade().unwrap().block_size(),
                );
