...
use crate::{
    fs::{
        ...
        namei::{filename_lookup, path_openat, Nameidata},
        path::Path,
        AT_FDCWD,
    },
    syscall::errno::Errno,
...
impl PasswdEntry {
    ...
    pub fn passwd_lookup(socket: &Socket, buf_len: usize) -> Result<Vec<u8>, Errno> {
            ...
            if let Some(d) = nscdrequest.db {
                ...
                *last_db_lock = Some(d);
                ...
                if nscdrequest.req_type == RequestType::GetpwNam {
                    ...
                    last_db_lock.clone().ok_or(Errno::EINVAL)? // 如果上一次也没有，就返回 EINVAL
                }
            }
        };
        ...
        if db == Database::Passwd {
            ...
            loop {
                log::trace!("[passwd_lookup] reading from /etc/passwd");
    ...
    fn parse_passwd_line(line: &str) -> Option<Self> {
        ...
        debug_assert!(
            parts.len() == 7,
            "passwd line should have 7 parts, got {}",
            parts.len()
        );
        if parts.len() != 7 {
