...
impl InodeOp for SocketInode {
    ...
    fn setxattr(
        ...
        Err(Errno::EPERM) // socket inode不支持设置扩展属性
    }
}
...
impl Socket {
    ...
    fn set_pachet_version(&self, version: u32) {
        self.packet_version
            .store(version, core::sync::atomic::Ordering::Release);
    }
        ...
        match socket_type {
            ...
            SocketType::SOCK_DGRAM => {
                if protocol != Protocol::UDP
                    && protocol != Protocol::IP
                    && protocol != Protocol::IPPROTO_UDPLITE
                {
                    return false;
                }
            }
...
impl FileOp for Socket {
    ...
    fn add_wait_queue(&self, tid: usize) {
        ...
        log::error!("[socket_add_wait_queue]:can not add wait queue for socket");
    }
    ...
    fn get_flags(&self) -> OpenFlags {
        ...
        log::error!("[socket_get_flags]:flag is {:?}", flag);
        flag
    }
    ...
    pub fn get(&self, rawsocket: &Socket, opt_addr: *mut u8, opt_len: *mut u32) {
        ...
        match self {
            ...
            TcpSocketOption::TCP_KEEPIDLE => {
                ...
                let value = socket.get_tcp_keepidle();
                let value = value.to_ne_bytes();
                log::error!("[TcpSocketOption::TCP_KEEPIDLE]value is {:?}", value);
                unsafe {
                    #[cfg(target_arch = "riscv64")]
                    copy_nonoverlapping(&value as *const u8, opt_addr, 4);
                    #[cfg(target_arch = "loongarch64")]
            ...
            TcpSocketOption::TCP_KEEPINTVL => {
                ...
                let value = socket.get_tcp_keepintvl();
                let value = value.to_ne_bytes();
                unsafe {
                    #[cfg(target_arch = "riscv64")]
                    copy_nonoverlapping(&value as *const u8, opt_addr, 4);
            ...
            TcpSocketOption::TCP_KEEPCNT => {
                ...
                let value = socket.get_tcp_keepcnt();
                let value = value.to_ne_bytes();
                unsafe {
                    #[cfg(target_arch = "riscv64")]
                    copy_nonoverlapping(&value as *const u8, opt_addr, 4);
...
impl Default for MessageHeaderRaw {
    fn default() -> Self {
        MessageHeaderRaw {
...
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct MMsgHdrRaw {
    pub hdr: MessageHeaderRaw,
    pub msg_len: u32,
}
impl Default for MMsgHdrRaw {
    fn default() -> Self {
        MMsgHdrRaw {
            hdr: MessageHeaderRaw::default(),
            msg_len: 0,
        }
    }
}
    ...
    pub fn set(&self, socket: &Socket, opt: &[u8]) -> SyscallRet {
        match self {
            SOL_PACKET_Option::PACKET_RX_RING => {
                ...
                let req: &TpacketReq3 = unsafe { &*(opt.as_ptr() as *const TpacketReq3) };
            ...
            SOL_PACKET_Option::PACKET_RESERVE => Ok(0),
            SOL_PACKET_Option::PACKET_VNET_HDR => Ok(0),
            SOL_PACKET_Option::PACKET_FANOUT => Ok(0),
            SOL_PACKET_Option::PACKET_FANOUT_ROLLOVER => Ok(0),
            SOL_PACKET_Option::TPACKET_V3 => Ok(0),
        }
    }
}
